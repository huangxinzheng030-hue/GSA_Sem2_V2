Shader "Custom/URP/InstagramGlassV3_SprayPatch_Unlit"
{
    Properties
    {
        ...
        _HighlightStrength ("View Highlight Strength", Range(0,5)) = 1.2
        _CenterBrightness ("Center Brightness", Range(0,3)) = 1.0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // Unlit: no scene lighting, no sun, only view-dependent shaping.
                half centerFade = pow(ndv, 1.25h);
                half3 bodyColor = lerp(_BaseColor.rgb * 0.06h, sprayColor, 0.68h);
                bodyColor *= lerp(0.75h, 1.0h, sprayMask);
                bodyColor *= lerp(0.8h, 1.0h, centerFade) * _CenterBrightness;

                half3 edgeColor = sprayColor * fresnel * _EdgeBrightness;
                half3 viewHighlight = lerp(half3(1,1,1), sprayColor, 0.35h) * pow(fresnel, 0.6h) * _HighlightStrength * 0.35h;

                half3 finalColor = bodyColor + edgeColor + viewHighlight;
